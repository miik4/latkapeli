...
var maali = false;
var langet = false;//länki laukaukselle apu muuttuja...

var liikutaNeulaaInterval = null;//setIntervalia varten.
...
var liikutaPystyPalkkiaInterval = null;//setIntervalia varten.
...
latkaApp.controller("latkaAppCtrl", function ($scope, $ionicPlatform, $window, $http, $cordovaMedia) {
    ...
    //functio neulan pysäytystä varten
    $scope.pysaytaNeula = function () {
    ...
    function liikutaNeulaa() {
        ...
        if ($scope.liikutaNeulaa_bool) {
            ...
            $scope.$apply();//tarvii tämän. Muuten ei paljo nuoli heilu :)
        }
        else {//kun neula on pysäytetty..
            
            clearInterval(liikutaNeulaaInterval);//pois, ettei jää turhaa taustalle jauhamaan
            //tehdään "mielikuvitus" maali alueet... eli millä arvoilla kiekko menee maaliin vaakatasossa.
            //kokonaisuudessaan "laukomis" alue on noittein neulan maxVasen ja maxOikean välillä eli -45 ja 45 välissä eli kokonaisuus alue 90, ellei sitten muutella max ja min...
    ...
    function liikutaPystyPalkkia() {
        ...
        if ($scope.liikutaPystyPalkkia_bool) {
        ...
        else {//kun pystypalkki on pysäytetty...
            
            clearInterval(liikutaPystyPalkkiaInterval);//pois, ettei jää turhaa taustalle jauhamaan
            
            //tehdään "mielikuvitus" maali alueet... eli millä arvoilla kiekko menee maaliin pystytasossa...
            //kokonaisuudessaan "laukomis" alue on noittein neulan max ja max välillä eli 0 (ylin asento) ja 97 (alin asento) välissä (liikkuvan laatikon koko 3% joten liikkumisalue 97% korkeudesta) 
            //eli kokonaisuus alue 97, ellei sitten muutella max ja min...
            ...
            else {//jos ei tule maalia ja peli loppuu..
    ...
    $scope.lisaaKantaanPisteetNimi=function() {
        $http({
            ...
            data : {nimi : $scope.nimi.teksti, pisteet : $scope.maalit}
                    ...
                    alert("Olet onnistuneesti jakanut pisteesi.");
                    //ei ammattitaitoisin vaihtoehto, mutta ionicPopup ei toiminut. 
                    //Ei pysty injektoimaan jostain syystä, vaan tulee pelkkää erroria (ionic bugi!?), joten ajan puutteen vuoksi alertilla.
                    $scope.maalit = 0;
                    pystyPalkki_arvo = 0;
                    neulan_arvo = 0;
                    $scope.pystyPalkki = { 
                ...
                , 
                function (response) { //jos tulee virhe niin ilmotus
